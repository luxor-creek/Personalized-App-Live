...
serve(async (req: Request) => {
  ...
  try {
    ...
    const response = await fetch(`${LEMLIST_API}/campaigns/${campaignId}`, {
    ...
    return new Response(JSON.stringify({ stats: campaign }), {
      status: 200,
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  } catch (error: any) {
    console.error("lemlist-get-campaign-stats error:", error.message);
